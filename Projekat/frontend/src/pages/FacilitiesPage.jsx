...
export default function FacilitiesPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Naziv ne smije biti samo brojevi
    if (/^\d+$/.test(formData.naziv.trim())) {
      showNotification('error', 'Naziv objekta ne smije sadržavati samo brojeve.');
      return;
    }

    // Adresa ne smije biti samo brojevi
    if (formData.adresa.trim() && /^\d+$/.test(formData.adresa.trim())) {
      showNotification('error', 'Adresa ne smije sadržavati samo brojeve.');
      return;
    }

    // 2.  Broj igrača na terenu mora biti realan (između 1 i 30)
    const playerCount = parseInt(formData.kapacitet, 10);
    if (!formData.kapacitet || isNaN(playerCount) || playerCount < 1 || playerCount > 30) {
      showNotification('error', 'Maksimalan broj igrača na terenu mora biti cijeli broj između 1 i 30.');
      return;
    }
  ...
  const resetForm = () => {
    setEditingId(null);
    setFormData({ naziv: '', adresa: '', opis: '', kapacitet: '12', status: 'AKTIVAN' });
  };
  ...
  return (
    <div className="min-h-screen bg-amber-50/60 font-sans pb-12">
      ...
      <div className="max-w-6xl mx-auto px-4 pt-8">
        ...
        {/* Notifikacije */}
        {statusMessage && (
          <div className={`mb-6 p-4 rounded-2xl border-2 font-bold text-sm shadow-sm transition-all ${statusMessage.type === 'success'
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="bg-white rounded-[32px] border-2 border-amber-100 shadow-sm p-6 w-full">
            ...
            <form onSubmit={handleSubmit} className="space-y-6" noValidate>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-start">
                ...
                <div className="flex flex-col">
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    placeholder="npr. 10 za futsal, 12 za fudbal"
                    className="w-full px-4 py-3 bg-white border-2 border-amber-100 rounded-2xl focus:border-orange-500 outline-none transition-all font-medium text-sm shadow-sm h-12"
                  />
                </div>
              </div>
              ...
              {/* NAPOMENA ISPOD FORME */}
              <p className="text-[11px] text-amber-800/60 bg-amber-50/50 p-3 rounded-xl border border-amber-100/70 italic leading-tight mt-2">
                * <strong>Napomena za broj igrača:</strong> Unesite ukupan broj osoba koje istovremeno aktivno igraju na terenu tokom jednog termina (npr. 10 igrača za futsal 5v5, 12 igrača za fudbal 6v6, ili 4 igrača za teniski debl).
              </p>
            </form>
          </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b-2 border-amber-100 bg-amber-50/40 text-xs font-black uppercase tracking-widest text-amber-900/60">
                      ...
                      {/* PROMIJENJENO: Naziv kolone u tabeli */}
                      <th className="pb-4 pt-2 px-6 text-center">Broj igrača</th>
                    ...
                    {facilities.map((facility) => (
                      <tr key={facility.objekatId} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="py-4 px-6 text-center font-bold text-slate-700">
                          {facility.kapacitet ? `${facility.kapacitet} igr.` : '/'}
                        </td>
